#!/usr/bin/env python3
"""
Clear all database records while keeping the schema/tables.

Usage:
    python clear_db.py --yes
    python clear_db.py --yes --keep-admin
"""
...
def clear_database(keep_admin=False):
    ...
    with app.app_context():
        ...
        with db.engine.begin() as connection:
            ...
            if keep_admin:
                print("Admin tables were preserved.")
...
def main():
    ...
    if not args.yes:
        print("Refusing to run without --yes. Example: python clear_db.py --yes")
        return

    clear_database(keep_admin=args.keep_admin)
